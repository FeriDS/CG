...
class Example(Base):
    ...
    def initialize(self):
        ...
        # variable used to multiply the directional lighting
        light_multiplier = 4;

        # four light sources
        ambient_light = AmbientLight(color=[0.1, 0.1, 0.1])
        self.scene.add(ambient_light)
        directional_light = DirectionalLight(color=[light_multiplier * 0.8,light_multiplier * 0.8,light_multiplier * 0.8], direction=[-1, -1, -2])
        ...
        self.speed = 7.27 * 0.01

    def update(self):
        # self.rig.update(self.input, self.delta_time)
        self.rig.rotate_y(math.pi * self.speed * self.delta_time, False)
