...
class Example(Base):
    ...
    def initialize(self):
        ...
        position_data = my_obj_reader("./guitarraFinal.obj")
        ...
        m = Matrix.make_translation(0, -3, -7.27 * 1.5)
    ...
    def update(self):
        ...
        if self.input.is_key_pressed('b'):
            m = Matrix.make_rotation_y(turn_amount)
            self.model_matrix.data = self.model_matrix.data @ m 
            # self.model_matrix.data[1][3] = -self.model_matrix.data[1][3] 


        if self.input.is_key_pressed('c'):
            m = Matrix.make_rotation_y(-turn_amount)
            self.model_matrix.data = self.model_matrix.data 
            # self.model_matrix.data[0][3] = -self.model_matrix.data[0][3]
